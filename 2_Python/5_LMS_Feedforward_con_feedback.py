...
class ANC_feedforward:
    def __init__(self, n_taps, mu, h_sec, f_sec):
        ...
        # Camino de feedback estimado
        self.f_sec = f_sec
        
        # Estado del filtro del camino secundario
        self.sec_state = np.zeros(len(h_sec)-1, dtype='float32')

         # Estado del filtro del feedback
        self.secf_state = np.zeros(len(f_sec)-1, dtype='float32')

    
    def process_block(self, d_block, e_block):

        # Filtrar bloque por el camino feedback estimado
        x_fb, self.secf_state = lfilter(
            self.f_sec, 
            1, 
            d_block, 
            zi=self.secf_state
        ...
        for i in range(len(d_block)):
            ...
            # Actualizar buffer de referencia FILTRADA por feedback y S(z)
...
plt.legend()
plt.subplot(3, 1, 3)
plt.plot(t, source_noise, label='Fuente de Ruido', fontsize=22)
